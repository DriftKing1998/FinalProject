...
def write_result(ortholog_genes):
    common_proteins = set({})

    groups = open(f'results/result_{qy.query.name.split("/")[-1].split(".",1)[0]}.tsv', 'w')
    ...
    genes = open(f'results/{qy.query.name.split("/")[-1].split(".",1)[0]}.genes.txt', 'w')
